...
def test_reset(setup_maze):
    ...
    assert initial_state == env.start, "L'état initial devrait correspondre au point de départ."

def test_move_up(setup_maze):
    """Test du mouvement vers le haut."""
    env = setup_maze
    env.start = (1, 0)  # Positionner l'agent en (1,0) pour tester le mouvement vers le haut
    env.reset()
